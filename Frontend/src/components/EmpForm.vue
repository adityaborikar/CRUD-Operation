...
onMounted(() => {
  axios
    .get(`http://localhost:3030/department/`, config)
    .then((response) => (dept_data.values = response.data.data));

  if (route.query.id) {
    axios
      .get(`http://localhost:3030/employee/${route.query.id}`, config)
      .then((response) => {
        data.name = response.data.name;
        data.contact = response.data.contact;
        data.dept_id = response.data.dept_id;
      });
  }
});
...
const add = () => {
  console.log(data);
  axios.post(`http://localhost:3030/employee/`, data, config);
  router.push({ path: `/home` });
};

const update = () => {
  axios.patch(`http://localhost:3030/employee/${route.query.id}`, data, config);
  router.push({ path: `/home` });
};
</script>
